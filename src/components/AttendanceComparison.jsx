...
export default function AttendanceComparison() {
  ...
  const [scopeFilter, setScopeFilter] = useState('all')   // 'all' | kampo id | ekklesia id
  const [scopeType,   setScopeType]   = useState('ekklesia') // 'ekklesia' | 'kampo'
  ...
  function applyFilter(records) {
    ...
    return records.filter(r => {
      ...
      if (!eid) return false
      if (scopeType === 'ekklesia') return eid === scopeFilter
      const ekk = EKKLESIAS.find(e => e.id === eid)
      return ekk?.kampoId === scopeFilter
    })
  }

  const filteredA = useMemo(() => applyFilter(recordsA), [recordsA, scopeFilter, scopeType])
  const filteredB = useMemo(() => applyFilter(recordsB), [recordsB, scopeFilter, scopeType])

  /* ── per-ekklesia counts ────────────────────────────────────────────── */
  const countA = useMemo(() => {
    const c = {}
    EKKLESIAS.forEach(e => { c[e.id] = 0 })
    filteredA.forEach(r => { const id = normalizeEkklesiaId(r); if (id) c[id] = (c[id] || 0) + 1 })
    return c
  }, [filteredA])
  ...
  const countB = useMemo(() => {
    const c = {}
    EKKLESIAS.forEach(e => { c[e.id] = 0 })
    filteredB.forEach(r => { const id = normalizeEkklesiaId(r); if (id) c[id] = (c[id] || 0) + 1 })
    return c
  }, [filteredB])
  ...
  /* ── chart: which ekklesias to show ────────────────────────────────── */
  const visibleEkklesias = useMemo(() => {
    if (scopeFilter === 'all') return EKKLESIAS
    if (scopeType === 'ekklesia') return EKKLESIAS.filter(e => e.id === scopeFilter)
    return EKKLESIAS.filter(e => e.kampoId === scopeFilter)
  }, [scopeFilter, scopeType])

  const barData = {
    labels: visibleEkklesias.map(e => e.name),
    datasets: [
      {
        label: lA,
        data: visibleEkklesias.map(e => countA[e.id] || 0),
      ...
      {
        label: lB,
        data: visibleEkklesias.map(e => countB[e.id] || 0),
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1.5">
            <div className="inline-flex rounded-lg border border-slate-200 overflow-hidden">
              {[['ekklesia', 'Ekklesia'], ['kampo', 'Kampo']].map(([val, label], i) => (
                <button
                  key={val}
                  type="button"
                  onClick={() => { setScopeType(val); setScopeFilter('all') }}
                  className={[
                    'px-2.5 py-1.5 text-xs font-semibold transition',
                    i > 0 ? 'border-l border-slate-200' : '',
                    scopeType === val ? 'bg-secondary text-white' : 'bg-white text-slate-600 hover:bg-slate-50',
                  ].join(' ')}
                >
                  {label}
                </button>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3 text-xs flex-wrap">
            ...
            {scopeFilter !== 'all' && (
              <span className="ml-auto inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-600 font-semibold">
                {scopeType === 'ekklesia'
                  ? EKKLESIAS.find(e => e.id === scopeFilter)?.name
                  : KAMPOS.find(k => k.id === scopeFilter)?.name
                }
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className={[
              ...
              diff > 0 ? 'border-emerald-200 bg-emerald-50' : diff < 0 ? 'border-red-200 bg-red-50' : 'border-slate-200 bg-slate-50',
            ].join(' ')}>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Difference</div>
              <div className={['text-3xl font-black', diff > 0 ? 'text-emerald-600' : diff < 0 ? 'text-red-600' : 'text-slate-400'].join(' ')}>
            ...
            <div className={[
              'rounded-xl border p-4',
              change === null ? 'border-slate-200 bg-slate-50' : change.dir === 'up' ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50',
            ].join(' ')}>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Change</div>
              <div className={['text-3xl font-black', change === null ? 'text-slate-300' : change.dir === 'up' ? 'text-emerald-600' : 'text-red-600'].join(' ')}>
          ...
          {/* Per-ekklesia breakdown table grouped by kampo */}
          <div>
            <div className="text-xs font-semibold text-slate-500 mb-2">Breakdown per Ekklesia</div>
            <div className="overflow-x-auto rounded-lg border border-slate-200">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="text-left font-semibold text-slate-500 px-4 py-2.5">Ekklesia</th>
                    <th className="text-left font-semibold text-slate-400 px-4 py-2.5">Kampo</th>
                    <th className="text-right font-semibold text-indigo-500 px-4 py-2.5">
                      <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-indigo-500" /> A</span>
                    </th>
                    <th className="text-right font-semibold text-amber-500 px-4 py-2.5">
                      <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-amber-500" /> B</span>
                    </th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {KAMPOS.map(kampo => {
                    ...
                    return ekks.map((e, ei) => {
                      const idx = EKKLESIAS.findIndex(x => x.id === e.id)
                      const a   = countA[e.id] || 0
                      const b   = countB[e.id] || 0
                      const d   = b - a
                      ...
                      return (
                        <tr key={e.id} className="hover:bg-slate-50 transition">
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2 font-semibold text-slate-700">
                              <span className="w-2.5 h-2.5 rounded-full shrink-0"
                                style={{ background: EKKLESIA_COLORS[idx] || '#888' }} />
                              {e.name}
                            </div>
                          </td>
                          <td className="px-4 py-2.5 text-slate-400">{ei === 0 ? kampo.name : ''}</td>
                          ...
                          <td className={['px-4 py-2.5 text-right font-bold', d > 0 ? 'text-emerald-600' : d < 0 ? 'text-red-500' : 'text-slate-400'].join(' ')}>
                            {d > 0 ? `+${d}` : d === 0 ? '0' : d}
                          </td>
                          <td className={['px-4 py-2.5 text-right font-semibold', c === null ? 'text-slate-400' : c.dir === 'up' ? 'text-emerald-600' : 'text-red-500'].join(' ')}>
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t-2 border-slate-200 font-bold">
                    ...
                    <td className={['px-4 py-2.5 text-right', diff > 0 ? 'text-emerald-600' : diff < 0 ? 'text-red-500' : 'text-slate-400'].join(' ')}>
                      {diff > 0 ? `+${diff}` : diff === 0 ? '0' : diff}
                    </td>
                    <td className={['px-4 py-2.5 text-right', change === null ? 'text-slate-400' : change.dir === 'up' ? 'text-emerald-600' : 'text-red-500'].join(' ')}>
                      {change === null ? '—' : change.dir === 'up' ? `↑ +${change.value}%` : `↓ −${change.value}%`}
                    </td>
                  </tr>
                </tfoot>
